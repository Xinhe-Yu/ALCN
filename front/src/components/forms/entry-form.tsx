...
interface EntryFormProps {
  entry?: EntryWithTranslations; // Optional for create mode
  onSave: (data: EntryFormData | Partial<EntryFormData>) => Promise<void>;
  onCancel: () => void;
  mode: 'create' | 'edit';
}
...
export default function EntryForm({ entry, onSave, onCancel, mode }: EntryFormProps) {
  const [isShortVersion, setIsShortVersion] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const { success, error } = useToast();

  // Form state - initialize with entry data for edit mode, empty for create mode
  const [formData, setFormData] = useState<EntryFormData>({
    primary_name: entry?.primary_name || '',
    original_script: entry?.original_script || '',
    language_code: (entry?.language_code as LanguageCode) || '' as LanguageCode,
    entry_type: (entry?.entry_type as EntryType | null) || null,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'create') {
        ...
        const updates: Partial<EntryFormData> = {};
        Object.keys(formData).forEach(key => {
          const fieldKey = key as keyof EntryFormData;
      ...
      console.error(`Failed to ${mode} entry:`, err);
      error('Error', `Failed to ${mode} entry`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const isEssentialFieldsValid = formData.primary_name?.trim() && formData.language_code;
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      required
                      placeholder="Enter the primary name of the entry"
                    />
                  </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-amber-500 focus:border-amber-500"
                      placeholder="Provide a clear definition of this entry"
                    />
                  </div>
                </div>
